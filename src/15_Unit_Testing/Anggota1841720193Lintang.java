...
public class Anggota1841720193Lintang {
    ...
    public ArrayList<Anggota1841720193Lintang> getByNamaAndAlamatAndTeleponLintang(String nama, String alamat, String telepon) {
        ArrayList<Anggota1841720193Lintang> ListAnggota = new ArrayList();
        ResultSet rs;
        if ((nama.trim().length() > 0) && (alamat.trim().length() > 0) && (telepon.trim().length() > 0)) {
            rs = DBHelper1841720193Lintang.selectQueryLintang("SELECT * FROM anggota WHERE nama = '" + nama + "'  AND alamat = '" + alamat + "'  AND telepon = '" + telepon + "'");

        } else if ((nama.trim().length() > 0) && (alamat.trim().length() > 0) && (telepon.trim().length() == 0)) {
            rs = DBHelper1841720193Lintang.selectQueryLintang("SELECT * FROM anggota WHERE nama = '" + nama + "'  AND alamat = '" + alamat + "'");
        } else if ((nama.trim().length() > 0) && (alamat.trim().length() == 0)) {
            rs = DBHelper1841720193Lintang.selectQueryLintang("SELECT * FROM anggota WHERE nama = '" + nama + "'");
        ...
        try {
            while (rs.next()) {
                Anggota1841720193Lintang ang = new Anggota1841720193Lintang();
                ang.setIdanggotaLintang(rs.getInt("idanggota"));
                ang.setNamaLintang(rs.getString("nama"));
                ang.setAlamatLintang(rs.getString("alamat"));
                ang.setTeleponLintang(rs.getString("telepon"));
                ListAnggota.add(ang);
            }
        } catch (Exception e) {
            e.printStackTrace();
